...
namespace Interaction_Streams_101
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            //StreamWriter sw = new StreamWriter(@"\\Mac\development\SuperEyeGame\test.txt", false);

            // 3. Get the gaze data!
            gazePointDataStream.GazePoint((x, y, ts) => Console.WriteLine("{0}\t{1}\t{2}", ts, x, y));

            //gazePointDataStream.GazePoint((x, y, ts) =>
            //{
            //    sw.WriteLine("{0}\t{1}\t{2}", ts, x, y);
